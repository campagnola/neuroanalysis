...
class Stimulus(object):
    ...
    def eval(self, time_values=None, n_pts=None, dt=None, sample_rate=None, t0=0, trace=None):
        """Return the value of this stimulus (as a Trace instance) at defined timepoints.
        ...
        * a set of arguments (t0, n_pts, dt/sample_rate) that can be converted to a Trace
        * a Trace instance
        ...
        n_pts : int | None
            The length of the Trace to generate. This argument requires either *dt* or 
            *sample_rate* to be given as well.
        dt : float | None
            Timestep between samples in the generated Trace (see Trace.dt). This argument
            requires *n_pts* to be given as well, and conflits with *sample_rate*.
        sample_rate : float | None
            Sampling rate of the generated Trace (see Trace.sample_rate). This argument
            requires *n_pts* to be given as well, and conflits with *dt*.
        t0 : float | None
            The first timepoint to evaluate (see Trace.t0). This argument requires *n_pts* and 
            either *dt* or *sample_rate* to be given as well.
        trace : Trace | None
            If given, then the evaluated result is *added* to the data array contained in the Trace.
        """
    ...
    def mask(self, trace=None, t0=None, n_pts=None, dt=None, sample_rate=None, time_values=None):
        """Return a Trace that contains boolean data indicating the regions of the trace
...
class Offset(Stimulus):
    ...
    def eval(self, **kwds):
        ...
        start_ind = trace.index_at(self.global_start_time)
...
class SquarePulse(Stimulus):
    ...
    def mask(self, **kwds):
        trace = Stimulus.mask(self, **kwds)
        start = self.global_start_time
        trace.time_slice(start, start+self.duration).data[:] = True
        return trace
...
class Ramp(Stimulus):
    ...
    def eval(self, **kwds):
        trace = Stimulus.eval(self, **kwds)
        start = self.global_start_time
        region = trace.time_slice(start, start + self.duration)
...
class Sine(Stimulus):
    ...
    def eval(self, **kwds):
        trace = Stimulus.eval(self, **kwds)
        start = self.global_start_time
        chunk = trace.time_slice(start, start+self.duration)
